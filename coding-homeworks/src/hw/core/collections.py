...
def even(some_not_even_list):
    return list(filter(lambda x: x % 2 == 0, not_even_list))
...
def get_ages(some_years_of_birth):
    return [2014 - year for year in some_years_of_birth]  # время в 14ом году остановилось :)
...
def count_symbols(some_string):
    chars = set(s)
    return map_keys_and_values(chars, (s.count(ch) for ch in chars))
